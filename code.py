...
def perform_clustering_and_rightsizing(metric_type: str, data: pd.DataFrame) -> pd.DataFrame:
    ...
    for week, group in df.groupby("Week"):
        ...
        # All 3 metrics must stay below 100% after x2
        # CpuMax MUST be included here (Postgres pattern)
        # Without CpuMax spiky clusters get wrong Downsize
        level1 = group[[f"{mt}MaxP95", f"{mt}Max", f"{mt}AvgP95"]].mul(2)
        level1.columns = [f"{mt}MaxP951level", f"{mt}Max1level", f"{mt}AvgP951level"]
    ...
    def assign_cluster(row: pd.Series) -> int:
        ...
        # CpuMaxP95 × 2, CpuMax × 2, CpuAvgP95 × 2
        # ALL must be < 100% (Postgres pattern)
        # CpuMax × 2 protects spiky clusters like cmsonc-eob-prod
        # CpuMax=75% × 2 = 150% > 100% → NoChange ✅
        c1_ok = (
            row.get(f"{mt}MaxP951level", 101) < 100
            and row.get(f"{mt}Max1level",    101) < 100
...
def recommendations(metric_type: str, data: pd.DataFrame, actual_sku: str, ordered_tiers: list[str]) -> list | None:
    ...
    # CpuMax spike protection:
    # If cluster hits high CPU spikes (>60%)
    # do NOT downsize even if CpuAvg is low
    # Downsizing would make spikes even worse
    elif _safe_quantile(df["CpuMax"], 0.95) > 60 if "CpuMax" in df.columns else False:
        final_idx = current_idx  # NoChange — spike protection
