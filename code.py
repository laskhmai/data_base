...
BEGIN
    ...
    -- GROUP BY Instance + Provider + Region for accurate values
    -- Each cluster gets correct ConnectionLimit for its provider/region
    -- =========================================
    SELECT
        Instance                            AS Sku,
        Provider,
        Region,
        MAX(CAST(vCores          AS FLOAT)) AS vCores,
        MAX(CAST(MemorySizeGB    AS FLOAT)) AS RAM_GB,
        MAX(CAST(ConnectionLimit AS FLOAT)) AS ConnectionLimit
    INTO #SkuConfig
    FROM [Analytics].[MongoDBMetaConfig]
    WHERE Tier NOT IN ('Free', 'Flex')
    GROUP BY Instance, Provider, Region;
    ...
    INNER JOIN #SkuConfig cs
        ...
        AND cs.Provider = a.ProviderName
        AND cs.Region   = a.RegionName
    LEFT  JOIN #SkuConfig wf
        ON  LEFT(r.RecommendedSku,
                 CHARINDEX(',', r.RecommendedSku + ',') - 1) = wf.Sku
        AND wf.Provider = a.ProviderName
        AND wf.Region   = a.RegionName
    LEFT  JOIN #SkuConfig lc
        ON  r.LowCpuSku  = lc.Sku
        AND lc.Provider  = a.ProviderName
        AND lc.Region    = a.RegionName
...
BEGIN
    ...
    ;WITH EffBase AS (
        SELECT
            ...
            -- NULL when no Low-CPU SKU available for this slice
            CASE WHEN nCpuAvgLowCpu IS NULL THEN NULL
            ELSE
                0.02 + (1.0 - 0.02) * (
                    1.0 / (1.0 + EXP(-0.09 * (nCpuAvgLowCpu - 50)))
                ) * (
                    0.5 + 0.5
                    * EXP(-0.05 * CASE WHEN nCpuMaxLowCpu < 25 THEN 25 - nCpuMaxLowCpu ELSE 0 END)
                    * (1 - CASE WHEN NULLIF(nCpuAvgP95LowCpu,0) IS NULL THEN 0
                                WHEN ABS(nCpuAvgLowCpu - nCpuAvgP95LowCpu) / nCpuAvgP95LowCpu > 1 THEN 1
                                ELSE ABS(nCpuAvgLowCpu - nCpuAvgP95LowCpu) / nCpuAvgP95LowCpu END)
                    * (1 - CASE WHEN NULLIF(nCpuMaxP95LowCpu,0) IS NULL THEN 0
                                WHEN ABS(nCpuMaxLowCpu - nCpuMaxP95LowCpu) / nCpuMaxP95LowCpu > 1 THEN 1
                                ELSE ABS(nCpuMaxLowCpu - nCpuMaxP95LowCpu) / nCpuMaxP95LowCpu END)
                )
            END                                             AS CpuEffLowCpu,

            -- Low-CPU Memory Efficiency
            -- NULL when no Low-CPU SKU available for this slice
            CASE WHEN nMemAvgLowCpu IS NULL THEN NULL
            ELSE
                0.02 + (1.0 - 0.02) * (
                    1.0 / (1.0 + EXP(-0.09 * (nMemAvgLowCpu - 50)))
                ) * (
                    0.5 + 0.5
                    * EXP(-0.05 * CASE WHEN nMemMaxLowCpu < 25 THEN 25 - nMemMaxLowCpu ELSE 0 END)
                    * (1 - CASE WHEN NULLIF(nMemAvgP95LowCpu,0) IS NULL THEN 0
                                WHEN ABS(nMemAvgLowCpu - nMemAvgP95LowCpu) / nMemAvgP95LowCpu > 1 THEN 1
                                ELSE ABS(nMemAvgLowCpu - nMemAvgP95LowCpu) / nMemAvgP95LowCpu END)
                    * (1 - CASE WHEN NULLIF(nMemMaxP95LowCpu,0) IS NULL THEN 0
                                WHEN ABS(nMemMaxLowCpu - nMemMaxP95LowCpu) / nMemMaxP95LowCpu > 1 THEN 1
                                ELSE ABS(nMemMaxLowCpu - nMemMaxP95LowCpu) / nMemMaxP95LowCpu END)
                )
            END                                             AS MemEffLowCpu
