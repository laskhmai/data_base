...
def load_spend_data(month: str) -> dict:
    ...
    Filters ONLY Instance rows (server cost)
    Excludes: Data Transfer, Backup Storage, Standard Storage
    These non-instance costs do NOT change when we resize the cluster

    Returns dict: {ClusterName: {ActualSpend, ActualSku, ActualCostPrHour}}
    ActualCostPrHour = SUM(Amount) / SUM(Quantity) for instance rows only
    This is the REAL price Atlas charged per server hour
    Used for more accurate savings calculation
    """
    sql = f"""
        SELECT
            s.Cluster                               AS ClusterName,
            ROUND(SUM(s.Amount), 2)                 AS ActualSpend,
            -- Most recent SKU instance (handles mid-month autoscale)
            (SELECT TOP 1 sp.Sku
            ...
            -- Actual cost per server hour from billing
            -- SUM(Amount) / SUM(Quantity) for instance rows only
            ROUND(
...
def process_cluster(cluster_key: int, cluster_name: str, instance_size: str,
    ...
    # More accurate than pure MetaConfig estimate since Atlas prices vary
