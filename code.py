...
-- DevOps  : 9009227
-- Version : 2.0  ← FIXED
-- Changes : Added #ClusterMetrics aggregation step
--           Collapses multiple shard primaries → 1 row per cluster
--           UPSERT match key changed from ProcessId → ClusterKey
-- =============================================
-- Source Tables:
--   Sections 1-9  : 5M tables (fast changing metrics)
--   Sections 10-12: 15M tables (stable metrics — no 5M available)
-- =============================================
-- 5M  : CPU Avg, CPU Max, Memory Resident,
--       Memory Available, Network In/Out Avg/Max,
--       Network Num Requests
-- 15M : Connections, Opc Query, Opc Insert
-- =============================================
-- PRE-REQUISITES:
--   Table [Metrics].[MongoDBRightsizingAggregated5Min] must exist
--   [Analytics].[MongoDBMetaConfig] must have ConnectionLimit column
...
BEGIN
    ...
    IF OBJECT_ID('tempdb..#ClusterMetrics') IS NOT NULL DROP TABLE #ClusterMetrics; -- NEW
    ...
    -- =========================================
    -- STEP 2 — NEW: Cluster Level Aggregation
    -- Collapses multiple shard primaries → 1 row per cluster per hour
    -- Problem: sharded clusters have multiple REPLICA_PRIMARY processes
    --          e.g. cdr-uat has shard-0 and shard-1 both as REPLICA_PRIMARY
    --          This step takes the worst-case metrics across all shards
    -- CPU    : MAX (worst shard drives sizing)
    -- Memory : MAX (worst shard drives sizing)
    -- Conn   : SUM (total connections = sum across all shards)
    ...
    SELECT
        ...
        -- CPU — worst shard drives sizing decision
        ...
        -- Memory — worst shard drives sizing decision
        ...
        -- Network — peak throughput across all shards
    ...
    -- — no longer meaningful at cluster level
    -- =========================================
...
-- =============================================
-- PRE-RUN: Truncate bad data first!
-- Run this BEFORE executing the proc
-- =============================================
...
-- =============================================
-- Verify — Run after proc completes
-- =============================================
...
-- 2. Confirm NO duplicates — this should return ZERO rows
SELECT
    ClusterKey,
    ClusterName,
    _date,
...
-- 3. Spot check cdr-uat — should be exactly 1 row per hour now
SELECT
    ClusterKey,
    ClusterName,
    _date,
