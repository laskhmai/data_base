...
CREATE OR ALTER PROCEDURE [Metrics].[usp_MongoDBRightsizingAggregatedMetrics5Min]
AS
BEGIN
    SET NOCOUNT ON;
    ...
    Agg AS (
        SELECT
            ...
            ROW_NUMBER() OVER (PARTITION BY [key], HourBucket ORDER BY (SELECT NULL))                 AS rn
        FROM Raw
    )
    SELECT
        [key],
    ...
    -- =============================================
    -- Section 2: CPU Max + P95 + Threshold Counts
    ...
    -- =============================================
    -- Section 3: Memory Resident + P95
    -- Source: MongoDB_Memory_Resident_5M (MB)
    -- =============================================
    ;WITH Raw AS (
        SELECT
            [key],
    ...
    -- =============================================
    -- Section 4: Memory Available
    -- Source: MongoDB_System_Memory_Available_5M (KB)
    -- =============================================
    ;WITH Raw AS (
        SELECT
            [key],
    ...
    -- =============================================
    -- Section 5: Network In Average
    -- Source: MongoDB_System_Network_In_5M (bytes/sec)
    -- =============================================
    ;WITH Raw AS (
        SELECT
            [key],
    ...
    -- =============================================
    -- Section 6: Network In Max
    -- Source: MongoDB_System_Network_In_Max_5M (bytes/sec)
    -- =============================================
    ;WITH Raw AS (
        SELECT
            [key],
    ...
    -- =============================================
    -- Section 7: Network Out Average
    -- Source: MongoDB_System_Network_Out_5M (bytes/sec)
    -- =============================================
    ;WITH Raw AS (
        SELECT
            [key],
    ...
    -- =============================================
    -- Section 8: Network Out Max
    -- Source: MongoDB_System_Network_Out_Max_5M (bytes/sec)
    -- =============================================
    ;WITH Raw AS (
        SELECT
            [key],
    ...
    -- =============================================
    -- Section 9: Network Requests Max
    ...
    -- Source: MongoDB_Connections_15M (no 5M available)
    -- =============================================
    ;WITH Raw AS (
        SELECT
            [key],
    ...
    -- =============================================
    -- Section 11: Opcounter Query Max
    -- Source: MongoDB_Opcounter_Query_15M (no 5M available)
    -- =============================================
    ;WITH Raw AS (
        SELECT
            [key],
    ...
    -- =============================================
    -- Section 12: Opcounter Insert Max
    -- Source: MongoDB_Opcounter_Insert_15M (no 5M available)
    -- =============================================
    ;WITH Raw AS (
        SELECT
            [key],
    ...
    -- =============================================
    -- Build keys spine from all metric sources
    -- =============================================
    ...
    -- =============================================
    -- Stage 1: Assemble per-process metrics
    -- Joins metric temp tables with process/cluster
    -- metadata and MetaConfig for sizing context.
    -- ConnectionLimit carried through for cluster-
    -- level utilization calculation in Stage 2.
    -- Connections zeroed for non-client processes
    -- (SECONDARY, CONFIG) to prevent inflation.
    -- =============================================
    SELECT
        p.ClusterKey,
        ...
        p.OrgKey,
        COALESCE(
            JSON_VALUE(cl.ReplicationSpecs, '$[0].regionConfigs[0].effectiveElectableSpecs.instanceSize'),
            JSON_VALUE(cl.ReplicationSpecs, '$[0].regionConfigs[0].electableSpecs.instanceSize')
        )                                                                           AS InstanceSize,
        JSON_VALUE(cl.ReplicationSpecs, '$[0].regionConfigs[0].providerName')       AS ProviderName,
        JSON_VALUE(cl.ReplicationSpecs, '$[0].regionConfigs[0].regionName')         AS RegionName,
        k.DateTimeEST,
        ...
        CASE WHEN COALESCE(m.MemorySizeGB, 0) > 0
            THEN ROUND((COALESCE(mr.MemResidentMax, 0) / (m.MemorySizeGB * 1024.0)) * 100, 2)
            ELSE 0 END                                                              AS MemResidentMaxPct,
        CASE WHEN COALESCE(m.MemorySizeGB, 0) > 0
            THEN ROUND((COALESCE(mr.MemResidentAvg, 0) / (m.MemorySizeGB * 1024.0)) * 100, 2)
            ELSE 0 END                                                              AS MemResidentAvgPct,
        CASE WHEN COALESCE(m.MemorySizeGB, 0) > 0
            THEN ROUND((COALESCE(mr.MemResidentP95, 0) / (m.MemorySizeGB * 1024.0)) * 100, 2)
            ELSE 0 END                                                              AS MemResidentP95Pct,
        ...
        -- Connections: only count client-facing process types
        -- REPLICA_PRIMARY and SHARD_MONGOS handle client connections
        -- SECONDARY and CONFIG handle internal replication only
        CASE WHEN p.ProcessType IN ('REPLICA_PRIMARY', 'SHARD_MONGOS')
            THEN COALESCE(cn.ConnectionsMax, 0) ELSE 0 END                         AS ConnectionsMax,
        CASE WHEN p.ProcessType IN ('REPLICA_PRIMARY', 'SHARD_MONGOS')
            THEN COALESCE(cn.ConnectionsAvg, 0) ELSE 0 END                         AS ConnectionsAvg,

        -- ConnectionLimit passed through for cluster-level calc in Stage 2
    ...
    LEFT JOIN  [Analytics].[MongoDBMetaConfig]   m
        ON     m.Instance = COALESCE(
                   JSON_VALUE(cl.ReplicationSpecs, '$[0].regionConfigs[0].effectiveElectableSpecs.instanceSize'),
                   JSON_VALUE(cl.ReplicationSpecs, '$[0].regionConfigs[0].electableSpecs.instanceSize'))
        AND    m.Provider = JSON_VALUE(cl.ReplicationSpecs, '$[0].regionConfigs[0].providerName')
        AND    m.Region   = JSON_VALUE(cl.ReplicationSpecs, '$[0].regionConfigs[0].regionName')
    ...
    -- =============================================
    -- Stage 2: Cluster-level aggregation
    -- Collapses all process rows into one row per
    -- cluster per hour. CPU and memory use worst-
    -- case (MAX) across processes. Connections are
    -- summed only from client-facing processes.
    -- ConnUtilizationPct is calculated here using
    -- total client connections vs cluster limit.
    -- =============================================
    SELECT
        ClusterKey,
        ClusterName,
    ...
    (
        ClusterKey, ClusterName,
        InstanceSize, ProviderName, RegionName, ProjectKey, OrgKey,
        DateTimeEST, _date, _hour, [type], businessHour,
    ...
    SELECT
        ...
        S.InstanceSize, S.ProviderName, S.RegionName, S.ProjectKey, S.OrgKey,
