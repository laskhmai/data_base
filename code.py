...
BEGIN
    ...
    --
    -- FIX (Jul-2026):
    -- Previous version partitioned by (ClusterKey, _date, _hour)
    -- meaning N = number of PROCESSES in 1 hour (usually 3-7)
    -- CEILING(3 × 0.95) = 3 = MAX → P95 always equalled MAX ❌
    --
    -- New version partitions by (ClusterKey, [type], businessHour)
    -- meaning N = number of HOURS across the full data window
    -- e.g. 20 days × 24 hours = 480 → genuine P95 ✅
    -- P95 value is then stamped onto every hour row for that cluster
    --
    -- Safety: if fewer than 20 hours exist, falls back to MAX
    -- Synapse compatible ✅
    -- =========================================

    -- Calculate P95 at CLUSTER level across all hours
    ;WITH RankedHourly AS (
        SELECT
            ClusterKey,
            [type],
            businessHour,
            _date,
            _hour,
            -- Per-hour cluster-level values (already MAX'd across processes)
            CpuAvg,
            CpuMax,
            MemResidentMaxPct,
            -- Rank across ALL hours for this cluster/type/businessHour
            ROW_NUMBER() OVER (
    ...
    ClusterP95 AS (
        -- Pick the value at 95th percentile HOUR position
        -- per cluster/type/businessHour
        SELECT
            ClusterKey,
            [type],
            businessHour,
            -- If < 20 hours available fall back to MAX
            MAX(CASE WHEN TotalHours >= 20
                          AND CpuAvgRn = CAST(CEILING(TotalHours * 0.95) AS INT)
                     THEN CpuAvg
                     WHEN TotalHours < 20
                     THEN CpuAvg  -- will take MAX below
                     END)                         AS CpuAvgP95Raw,
            MAX(CASE WHEN TotalHours >= 20
                          AND CpuMaxRn = CAST(CEILING(TotalHours * 0.95) AS INT)
                     THEN CpuMax
                     WHEN TotalHours < 20
                     THEN CpuMax
                     END)                         AS CpuMaxP95Raw,
            MAX(CASE WHEN TotalHours >= 20
                          AND MemRn = CAST(CEILING(TotalHours * 0.95) AS INT)
                     THEN MemResidentMaxPct
                     WHEN TotalHours < 20
                     THEN MemResidentMaxPct
                     END)                         AS MemP95Raw,
    ...
    -- Stamp cluster-level P95 onto every hour row
    SELECT
        f.ClusterKey,
        f._date,
        f._hour,
        f.[type],
        f.businessHour,
        p.CpuAvgP95Raw                            AS CpuAvgP95,
        p.CpuMaxP95Raw                            AS CpuMaxP95,
        p.MemP95Raw                               AS MemResidentP95Pct
    INTO #TrueClusterP95
    FROM #FinalMetrics f
    JOIN ClusterP95 p
        ON  p.ClusterKey   = f.ClusterKey
        AND p.[type]       = f.[type]
        AND p.businessHour = f.businessHour;
