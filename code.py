...
def _trend(df: pd.DataFrame, feature: str) -> list[str]:
    ...
    for i in range(len(weekly) - 1):
        pair = weekly.iloc[i : i + 2][feature]
        if pair.isna().all():
            trend_status.append("No trend")
            continue
        stl = STL(pair.fillna(0), period=2)
        res = stl.fit()
        if res.trend.iloc[-1] > res.trend.iloc[0]:
            trend_status.append("Increasing")
        elif res.trend.iloc[-1] < res.trend.iloc[0]:
...
def process_cluster(cluster_key: int, cluster_name: str, instance_size: str, provider_name: str, region_name: str, config: dict, metacache: dict) -> dict | None:
    ...
    if actual_sku not in ordered_tiers:
        # If SKU not found in region/provider metaconfig, skip safely.
        return None
    ...
    return {
        ...
        "CpuRec": cpu_sku,
        "MemRec": mem_sku,
        "ConnectionsRec": conn_sku,
        "OverallRecommendation": overall_sku,
        "Comment": comment,
...
def insert_in_chunks(df: pd.DataFrame, chunk_size: int = 100):
    for start in range(0, len(df), chunk_size):
        chunk = df.iloc[start : start + chunk_size].copy()
        payload = [tuple(row[col] for col in INSERT_COLUMNS) for _, row in chunk.iterrows()]
...
if __name__ == "__main__":
    ...
    end_dates = [((last_month.replace(day=1) + relativedelta(months=1)) - timedelta(days=1)).strftime("%Y-%m-%d")]
    ...
    if not out_df.empty:
        ...
        print(out_df[["ClusterName", "CurrentSKU", "OverallRecommendation", "Comment"]].to_string())
        insert_in_chunks(out_df)
        print("Results inserted into [Metrics].[MongoDBRightsizingRecommendations].")
    else:
