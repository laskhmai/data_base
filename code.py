...
def connections_recommendation(data: pd.DataFrame, actual_sku: str,
                                ...
                                shard_count: int = 1) -> tuple[str, str]:
    """
    Neeraja: connection limit in MetaConfig is PER SHARD.
    For sharded clusters: total limit = per-shard limit × shard count
    """
    if data.empty:
        return actual_sku, ""
...
def component_comment(cpu_idx: int, mem_idx: int, conn_idx: int,
                       current_idx: int, mem_skipped: bool = False) -> str:
    """
    ...
    mem_skipped = True → Memory Excluded (not analyzed)
    """
    ...
    if mem_skipped:
        pass  # exclude from comment entirely
    else:
...
def process_cluster(cluster_key: int, cluster_name: str, instance_size: str,
    ...
    conn_sku, conn_comment = connections_recommendation(
