    ...
    BEGIN TRY
        ...
        (
            SELECT
                ...
                -- NO STUFF needed for GCP - no GUID prefix
        ...
        (
            SELECT
                ...
                , reservation_identifier_cost =
                    '{' + STRING_AGG(
                        CAST(CONCAT('"', ri_key, '":', CONVERT(VARCHAR(50), ri_spend))
                        AS nvarchar(max)),
                        ','
                    ) + '}'
            FROM ri_sum
        ...
        (
            SELECT
                ...
                , humana_applicationid_cost =
                    '{' + STRING_AGG(
                        CAST(CONCAT('"', app_key, '":', CONVERT(VARCHAR(50), app_spend))
                        AS nvarchar(max)),
                        ','
                    ) + '}'
            FROM app_sum
        ...
        (
            SELECT
                ...
                , humana_resourceid_cost =
                    '{' + STRING_AGG(
                        CAST(CONCAT('"', hr_key, '":', CONVERT(VARCHAR(50), hr_spend))
                        AS nvarchar(max)),
                        ','
                    ) + '}'
            FROM hr_sum
