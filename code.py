...
def reconcile_autoscaling(current_idx: int, rec_idx: int,
    ...
    if rec_idx > current_idx:
        if max_idx != -1 and current_idx < max_idx:
            ...
            return out
        new_max_idx          = max(rec_idx, max_idx if max_idx != -1 else rec_idx)
        out["action"]        = "RaiseMax"
        out["effective_idx"] = new_max_idx
        out["rec_max_sku"]   = ordered_tiers[new_max_idx]
        out["note"]          = f"Sustained high load at max ({max_sku}) — raise max ceiling"
        return out
    ...
    if rec_idx < current_idx:
        ...
        if scale_down_enabled:
            if min_idx != -1 and rec_idx < min_idx:
                out["action"]        = "LowerMin"
                out["effective_idx"] = rec_idx
                out["rec_min_sku"]   = ordered_tiers[rec_idx]
                out["note"]          = f"Usage below floor ({min_sku}) — lower min to reduce cost"
                return out
        ...
        out["effective_idx"] = rec_idx
        out["rec_min_sku"]   = ordered_tiers[rec_idx]
        out["note"]          = "Scale-down disabled — enable it and lower min to recover cost"
        return out
