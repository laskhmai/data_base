-- Create empty copy of Recommendations table
SELECT *
INTO [Metrics].[MongoDBRightsizingRecommendations_STL]
...
def _trend(df: pd.DataFrame, feature: str) -> list[str]:
    ...
    Matches Postgres pattern exactly.

    STL separates data into:
      Trend     → general up/down direction
      Seasonal  → repeating weekly patterns (e.g. Monday batch jobs)
      Remainder → random noise/outliers

    This means:
      Predictable weekly spikes → seasonal (not flagged as risky)
      Genuine increasing trend  → flagged correctly
      Random one-off spikes     → remainder (ignored)

    Falls back to simple slope if STL fails or too little data.
    """
    ...
    for i in range(len(weekly) - 1):
        week_data = weekly.iloc[i : i + 2][feature].fillna(0)

        # Need at least 2 points for STL
        if len(week_data) < 2:
            trend_status.append("No trend")
            continue

        try:
            # STL decomposition — same as Postgres check_trend_direction()
            # period=2 as Postgres uses (2 observations per window)
            stl    = STL(week_data, period=2, robust=True)
            result = stl.fit()

            # Check trend direction from STL result
            if result.trend.sum() != 0:
                if result.trend.iloc[-1] > result.trend.iloc[0]:
            ...
            # Fallback to simple slope if STL fails
            # (e.g. insufficient data, constant values)
            y     = week_data.values
            x     = np.arange(len(y))
            try:
...
def recommendations(metric_type: str, data: pd.DataFrame, actual_sku: str, ordered_tiers: list[str]) -> list | None:
    ...
    if any(s == 3 for s, _ in weekly_actions) and peak_val > 80:
        ...
        # At least one week was risky, but not extreme enough to trigger Upsize.
        # This is an ambiguous/uncertain case — default to NoChange (stay safe)
        # instead of Downsize. Previously this fell through to Downsize, which
        # is how clusters with real spikes (e.g. cmsonc-eob-prod, 75% peak)
        # were getting recommended for Downsize despite the risk.
