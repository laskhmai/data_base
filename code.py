...
def recommendations(metric_type: str, data: pd.DataFrame, actual_sku: str, ordered_tiers: list[str]) -> list | None:
    ...
    # Fix C: PeakCpuMax safety check
    # Neeraja: "Have you checked CPU Max?"
    # Even if P95 looks safe, if raw peak × 2 > 100%
    # cluster would hit capacity after downsize → NoChange
    # e.g. PeakCpuMax=57% × 2 = 114% → blackout risk! ❌
    if final_idx < current_idx:
        peak_cpu_col = f"{mt}Max"
        peak_cpu_check = _safe_max(df[peak_cpu_col]) \
                         if peak_cpu_col in df.columns else 0.0
        if peak_cpu_check * 2 >= 100:
            final_idx = current_idx  # NoChange — peak CPU unsafe after downsize
...
def load_spend_data(month: str) -> dict:
    ...
    Neeraja: need SKU and cost from Spend table.

    Also builds discount lookup per SKU + Provider
    AZURE discount ~0.32, GCP discount ~0.29

    Returns dict: {
        ClusterName: {ActualSpend, ActualSku, ActualCostPrHour},
        "__discounts__": {(Provider, SkuName): discount_ratio}
    }
    """
    sql = f"""
        SELECT
            ...
            (SELECT TOP 1 sp.Sku
             ...
             AND   FORMAT(CAST(sp.UsageDate AS DATE), 'yyyy-MM') = '{month}'
             AND   sp.Sku LIKE '%Instance%'
    ...
    # Build discount lookup: actual/list ratio per Provider+SKU
    discount_sql = f"""
        SELECT
            REPLACE(REPLACE(REPLACE(s.Sku,
                'Atlas ',''),
                ' Instance - Azure',''),
                ' Instance - GCP','')           AS SkuName,
            CASE
                WHEN s.Sku LIKE '%Azure%' THEN 'AZURE'
                WHEN s.Sku LIKE '%GCP%'   THEN 'GCP'
                ELSE 'AZURE'
            END                                     AS Provider,
            ROUND(
                SUM(CASE WHEN s.Sku LIKE '%Instance%'
        ...
        JOIN [Analytics].[MongoDBMetaConfig] m
            ON  m.Instance = REPLACE(REPLACE(REPLACE(s.Sku,
                    'Atlas ',''),
                    ' Instance - Azure',''),
                    ' Instance - GCP','')
            AND m.Provider = CASE
        ...
        GROUP BY
            ...
            m.CostPrHour
    """
    discount_df = fetch_data(discount_sql)

    discount_lookup = {}
    if not discount_df.empty:
        for _, row in discount_df.iterrows():
            key = (str(row["Provider"]).upper(),
                   str(row["SkuName"]).upper())
            ratio = row["DiscountRatio"]
            if ratio and 0 < float(ratio) <= 1.0:
                discount_lookup[key] = float(ratio)

    if df.empty:
        return {"__discounts__": discount_lookup}

    result = {"__discounts__": discount_lookup}
    for _, row in df.iterrows():
        result[row["ClusterName"]] = {
            "ActualSpend":      row["ActualSpend"],
            "ActualSku":        row["ActualSku"],
            "ActualCostPrHour": row["ActualCostPrHour"]
        }
    return result
...
def build_cluster_inventory_query(start_date: str, end_date: str) -> str:
    return f"""
        ...
        SELECT DISTINCT
            ...
            c.ProjectKey,
            -- Autoscaling settings from ReplicationSpecs JSON
            JSON_VALUE(c.ReplicationSpecs,
                '$[0].regionConfigs[0].autoScaling.compute.enabled')
                                                    AS AutoScaleEnabled,
            JSON_VALUE(c.ReplicationSpecs,
                '$[0].regionConfigs[0].autoScaling.compute.scaleDownEnabled')
                                                    AS ScaleDownEnabled,
            JSON_VALUE(c.ReplicationSpecs,
...
# ===========================================================================
# 8B. AUTO-SCALING RECONCILIATION
# ===========================================================================

def _as_bool(val) -> bool:
    """JSON_VALUE returns 'true'/'false' strings (or None) — coerce to bool."""
    return str(val).strip().lower() in ("true", "1", "yes")


def reconcile_autoscaling(current_idx: int, rec_idx: int,
                          min_sku: str, max_sku: str,
                          scale_down_enabled: bool,
                          ordered_tiers: list[str]) -> dict:
    """
    Reinterpret a fixed-SKU recommendation against the auto-scaling band.

    UPSIZE wanted:
      actual < max → NoChange (Auto): Atlas scales up itself ✅
      actual >= max → RaiseMax: ceiling too low ✅

    DOWNSIZE wanted:
      actual > min → NoChange (Auto): Atlas scales down itself ✅
      actual = min + scaledown ON  → LowerMin: drop the floor ✅
      actual = min + scaledown OFF → EnableScaleDown: stuck at peak ✅

    NOCHANGE:
      → NoChange (Auto) ✅
    """
    min_idx = tier_index(min_sku, ordered_tiers) if min_sku else -1
    max_idx = tier_index(max_sku, ordered_tiers) if max_sku else -1

    out = {
        "action":        "NoChange (Auto)",
        "effective_idx": current_idx,
        "rec_min_sku":   min_sku or None,
        "rec_max_sku":   max_sku or None,
        "note":          "Auto-scaling manages size within band",
    }

    # CPU HIGH — engine wants to grow
    if rec_idx > current_idx:
        if max_idx != -1 and current_idx < max_idx:
            # Below ceiling — Atlas handles it automatically
            out["note"] = "Load high but below max — Atlas will scale up automatically"
            return out
        # At/above ceiling and still hot → raise ceiling by ONE step only
        # Use max_idx + 1 not rec_idx (rec may jump too far e.g. M80→M200)
        new_max_idx          = min(
            (max_idx + 1) if max_idx != -1 else current_idx + 1,
            len(ordered_tiers) - 1
        )
        out["action"]        = "RaiseMax"
        out["effective_idx"] = new_max_idx
        out["rec_max_sku"]   = ordered_tiers[new_max_idx]
        out["note"]          = f"Sustained high load at max ({max_sku}) — raise max to {ordered_tiers[new_max_idx]}"
        return out

    # CPU LOW — engine wants to shrink
    if rec_idx < current_idx:
        if min_idx != -1 and current_idx > min_idx:
            # Above minimum — Atlas will scale down automatically
            out["note"] = "Under-utilized but above min — Atlas will scale down automatically"
            return out
        # At minimum
        if scale_down_enabled:
            if min_idx != -1 and rec_idx < min_idx:
                # Lower floor by ONE step only — not full rec_idx jump
                new_min_idx          = max(min_idx - 1, 0)
                out["action"]        = "LowerMin"
                out["effective_idx"] = new_min_idx
                out["rec_min_sku"]   = ordered_tiers[new_min_idx]
                out["note"]          = f"Usage below floor ({min_sku}) — lower min to {ordered_tiers[new_min_idx]}"
                return out
            out["note"] = "Under-utilized at min — Atlas will scale down automatically"
            return out
        # Scale-down disabled — cluster stuck at peak size
        new_min_idx          = max(min_idx - 1, 0) if min_idx != -1 else max(rec_idx, 0)
        out["action"]        = "EnableScaleDown"
        out["effective_idx"] = new_min_idx
        out["rec_min_sku"]   = ordered_tiers[new_min_idx]
        out["note"]          = f"Scale-down disabled — enable it and lower min to {ordered_tiers[new_min_idx]}"
        return out
...
def process_cluster(cluster_key: int, cluster_name: str, instance_size: str,
                    ...
                    metacache: dict, spend_data: dict = None,
                    autoscale_enabled: bool = False,
                    scale_down_enabled: bool = False,
                    min_instance_size: str = "",
                    max_instance_size: str = "") -> dict | None:
    ...
    discount_lookup        = spend_data.get("__discounts__", {})

    # If same SKU → savings = 0 (no change made)
    if overall_sku == actual_sku:
        estimated_monthly_savings = 0.0

    # Use actual billing rate + per-SKU per-Provider discount
    elif actual_cost_hr and actual_cost_hr > 0 and current_cost > 0:
        # Look up discount for current Provider + SKU
        # e.g. (AZURE, M30) → 0.3279, (GCP, M30) → 0.2828
        sku_key  = (prov, actual_sku.upper())
        discount = discount_lookup.get(sku_key, None)

        if discount and 0 < discount <= 1.0:
            # Apply same Provider discount to recommended SKU
            # Assumes Atlas applies consistent discount per provider
            estimated_recommended     = recommended_cost * discount
            estimated_monthly_savings = (actual_cost_hr - estimated_recommended) \
                                        * hours_in_month
        else:
            # No discount found → fall back to MetaConfig comparison
    ...
    # Reconcile CPU recommendation against the band
    # -----------------------------------------------------------------------
    min_sku_norm       = str(min_instance_size).upper().strip() if min_instance_size else ""
    max_sku_norm       = str(max_instance_size).upper().strip() if max_instance_size else ""
    ...
    if autoscale_enabled:
        recon = reconcile_autoscaling(
            current_idx, overall_idx,
            min_sku_norm, max_sku_norm,
            scale_down_enabled, ordered_tiers,
        )
        autoscale_action = recon["action"]
        rec_min_sku      = recon["rec_min_sku"]
        rec_max_sku      = recon["rec_max_sku"]
        autoscale_note   = recon["note"]
        overall_idx      = recon["effective_idx"]
    ...
    return {
        ...
        "Action":                  action,
        "AutoScaleEnabled":        1 if autoscale_enabled else 0,
        "ScaleDownEnabled":        1 if scale_down_enabled else 0,
        "MinInstanceSize":         min_sku_norm or None,
        "MaxInstanceSize":         max_sku_norm or None,
        "RecommendedMinSku":       rec_min_sku,
        "RecommendedMaxSku":       rec_max_sku,
        "SavingsBasis":            "Indicative (AutoScale)" if autoscale_enabled else "Fixed",
        "AuditUtc":                audit_utc,
    }
...
REC_TABLE = "[Metrics].[MongoDBRightsizingRecommendations]"
...
INSERT_SQL = f"""
    INSERT INTO {REC_TABLE} (
        ...
        LowCpuSku, LowCpuSavings, Action,
        AutoScaleEnabled, ScaleDownEnabled,
        MinInstanceSize, MaxInstanceSize,
        RecommendedMinSku, RecommendedMaxSku,
        SavingsBasis, AuditUtc
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
"""
    ...
    for month, configs in month_dict.items():
        ...
        for config in configs:
            ...
            for _, row in clusters.iterrows():
                ...
                # Autoscaling settings from Clusters.ReplicationSpecs
                autoscale_on  = _as_bool(row.get("AutoScaleEnabled"))
                scaledown_on  = _as_bool(row.get("ScaleDownEnabled"))
                min_size      = str(row.get("MinInstanceSize") or "")
                max_size      = str(row.get("MaxInstanceSize") or "")
                ...
                result = process_cluster(
                    ...
                    scale_down_enabled=scaledown_on,
                    min_instance_size=min_size,
                    max_instance_size=max_size,
                )
                if result is None:
                    continue
